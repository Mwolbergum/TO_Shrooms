...
# For KNN, the x and y variables need to be separated from each other. Our y-variable, "is_canceled", is the 2nd column in the dataset, so I am filtering the columns in our train and test data by the that value.
...
confusionMatrix(as.factor(dt_pred_binary), as.factor(shrooms_test$is_poisonous), positive = "1")
```
...
rf_pred_binary <- predict(rf_model,shrooms_test)
...
confusionMatrix(as.factor(knn_model), as.factor(shrooms_norm_test_y), positive = as.character(1))
```
...
svm_pred_vanilla_binary <- ifelse(svm_pred_vanilla >= 0.5, 1, 0)
...
shrooms_preds <- data.frame(log = logistic_pred, 
                          ...
                          ann = ann_pred, 
                          svm = svm_pred_vanilla, 
                          true = shrooms_test$is_canceled)
# Missing random forest, and it's not working but I cant figure out why :)
```
